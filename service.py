...
config = {
    ...
    "mongodb_URI": "mongodb://[username:password@]host[:port][/[defaultauthdb][?options]]",  # noqa:501
    "post_config": {"api_url": "https://api.example.com", "api_token": ""},
}
...
def get_config(user_id_list=None):
    ...
    handle_config_renaming(
        current_config, oldName="filter", newName="only_crawl_original"
    )
    handle_config_renaming(
        current_config, oldName="result_dir_name", newName="user_id_as_folder_name"  # noqa:501
    )
...
def refresh():
    ...
    if not user_id_list or not isinstance(user_id_list, list):
        return jsonify({"error": "Invalid user_id_list parameter"}), 400
...
def get_task_status(task_id):
    ...
    response = {"state": task["state"], "progress": task["progress"]}
        ...
        try:
            ...
            if not running_task:
                ...
                with task_lock:
                    global current_task_id
                    current_task_id = task_id
                executor.submit(run_refresh_task, task_id, config["user_id_list"])  # noqa:501
                logger.info(f"Scheduled task {task_id} started")

            time.sleep(600)  # 10分钟间隔
        except Exception as e:
            logger.exception("Schedule task error", e)
