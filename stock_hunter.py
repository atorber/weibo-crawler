...
def get_stock_data(text):
    ...
    4. 请将找到的股票名称和标签按照股票名称:标签的格式返回
    
    有股票信息时的返回如下文本格式必须以"{"开头，以"}"结束：
    {
    "万兴科技": [
        "文生视频",
        "Sora"
...
def get_stocks(weibo_user=None):
    try:
        ...
        if weibo_user is not None:
            ...
            cursor.execute("SELECT * FROM stock WHERE screen_name IN ({}) ORDER BY created_at DESC".format(', '.join('?' * len(weibo_user))), weibo_user)
        else:
...
def update_stock(id, stock):
    try:
        ...
        cursor = conn.cursor()
        # 按id更新微博
        cursor.execute("UPDATE stock SET text=? WHERE id=?", (stock['text'], id))
        conn.commit()
        conn.close()
...
def get_blacklist(table_id_blacklist):
    ...
    all_blacklist_items = json.loads(all_blacklist_response.raw.content)['data']['items']
    blacklist = []
    blacklist_map = {}
...
def update_stock(table_name_stock, weibo_user=None):

    table_id_stock = api.get_table_id(table_name_stock)

    weibos = get_weibos()
    logging.info('获取全部微博：%d', len(weibos))
    # logging.info(weibos)
    ...
    logging.info('all_records_items: %s', all_records_items)
    ...
    for key in table3.keys():
        ...
        if key in blacklist:
            ...
            new_name = blacklist_map[key] if key in blacklist_map.keys() else None
        ...
        if name not in all_names:
            ...
            logging.info('添加记录：%s', name)
            logging.info('添加记录：%s', stock)
            ...
            # 合并标签
            old_tag = record['fields']["标签"] if '标签' in record['fields'].keys() else []
            tag = old_tag + tag
            # 移除screen_name标签
    ...
    if len(new_stocks) > 0:
        logging.info('添加记录：%d', len(new_stocks))
    else:
        logging.info('无需添加')

    if len(new_records) > 0:
        api.batch_update_records(table_id_stock, new_records)
        logging.info('更新记录：%d', len(new_records))
    else:
        logging.info('无需更新')
