...
def get_stocks(weibo_user=None):
    try:
        ...
        if weibo_user is not None:
            ...
            cursor.execute(
                "SELECT * FROM stock WHERE screen_name IN ({}) ORDER BY created_at DESC".format(
                    ", ".join("?" * len(weibo_user))
                ),
                weibo_user,
            )
...
def get_blacklist(table_id_blacklist):
    ...
    all_blacklist_items = json.loads(all_blacklist_response.raw.content)["data"][
        "items"
    ]
    blacklist = []
    blacklist_map = {}
...
def update_stock(table_name_stock, weibo_user=None):
    ...
    for key in table3.keys():
        ...
        if name not in all_names:
            ...
            # 合并标签
            old_tag = (
                record["fields"]["标签"] if "标签" in record["fields"].keys() else []
            )
            tag = old_tag + tag
            # 移除screen_name标签
